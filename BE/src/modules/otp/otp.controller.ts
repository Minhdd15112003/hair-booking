...
export class OtpController {
    ...
    async getOTP(@Body() getOTPDto: GetOTP_Dto, @Req() req: Request, @Res() res: Response) {
        ...
        const existsOTP = await PrismaDB.otp.findUnique({ where: { phone_number: phone } });
        ...
        const now = new Date();
        const expiresAt = addMinutesToDate(now, 2);

        const otp_code = Math.floor(100000 + Math.random() * 900000).toString();
        const createdOTP = await PrismaDB.otp.create({ data: { otp: otp_code, phone_number: phone, expiresAt } });
    ...
    async verifyOTP(@Body() verifyOTP_Dto: VerifyOTP_Dto, @Res() res: Response) {
        ...
        const existsOTP = await PrismaDB.otp.findUnique({
            where: {
                phone_number: phone,
                otp: otp_code,
                expiresAt: {
        ...
        await PrismaDB.otp.delete({ where: { id: existsOTP.id } });
