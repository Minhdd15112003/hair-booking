...
export class CustomerController {
    ...
    @Get('banned')
    async getBannedCustomers(@Res() res: Response) {
        try {
            const bannedCustomers = await PrismaDB.user.findMany({
                where: { deleted: true, role: Roles.CUSTOMER },
            });

            return res.json({ success: true, data: bannedCustomers });
        } catch (error) {
            console.log(error);
    ...
    async restore(@Param('id') id: string) {
        try {
            const restoredCustomer = await PrismaDB.user.update({
                where: { id },
                data: { deleted: false },
            });
